...
SELECT * FROM PEDINDO WHERE ID = 130;
DELETE FROM PEDINDO WHERE ID = 130;
SELECT * FROM FORNECENDO;

SELECT SUM(P.VALOR * PE.QUANTIDADE) AS VALOR FROM PEDINDO AS PE INNER JOIN PRODUTO AS P ON P.ID = PE.ID_PRODUTO WHERE PE.FINALIZADO = 'SIM' AND PE.ID_DETALHE = 51
UPDATE DETALHE_PEDIDO SET VALOR = $valor WHERE ID = $finalizado
