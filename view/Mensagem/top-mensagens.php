<?php
	...
	try{
		...
		$query = "SELECT
					...
					D.STATUS AS STATUS,
					(SELECT COUNT(*) FROM MENSAGEM AS M WHERE M.ID_DE = C.ID AND M.DE = 'CLIENTE' AND ID_DE = C.ID AND (M.DESCRICAO = 3 OR M.DESCRICAO = 2 )) AS JA
				FROM
					DETALHE_PEDIDO AS D
				INNER JOIN
					CLIENTE AS C
	...
	// 	$query = "SELECT F.NOME, F.ID, (SELECT COUNT(*) FROM MENSAGEM AS M WHERE M.ID_DE = F.ID AND M.DE = 'FORNECEDOR' AND ID_DE = F.ID AND M.DESCRICAO = 4) AS JA FROM DETALHE_FORNECEDOR AS D INNER JOIN FORNECEDOR AS F ON D.ID_FORNECEDOR = F.ID WHERE VENCIMENTO < NOW()";
