...
SELECT F.NOME, F.ID, (SELECT COUNT(*) FROM MENSAGEM AS M WHERE M.ID_DE = F.ID AND M.DE = 'FORNECEDOR' AND ID_DE = F.ID AND M.DESCRICAO = 4) AS JA FROM DETALHE_FORNECEDOR AS D INNER JOIN FORNECEDOR AS F ON D.ID_FORNECEDOR = F.ID WHERE VENCIMENTO < NOW()
...
SELECT M.ID, M.DE, M.ASSUNTO, T.DESCRICAO, to_char(M.DATA_MSG, 'DD/MM/YYYY') as DATA_MSG, M.ID_DE FROM MENSAGEM AS M INNER JOIN TIPO_MENSAGEM AS T ON M.DESCRICAO = T.ID
SELECT M.ID AS ID, M.DE AS DE, M.ASSUNTO AS ASSUNTO, T.DESCRICAO AS DESCRICAO, to_char(M.DATA_MSG, 'DD/MM/YYYY') as DATA_MSG, M.ID_DE AS ID_DE FROM MENSAGEM AS M INNER JOIN TIPO_MENSAGEM AS T ON M.DESCRICAO = T.ID
